...
const citySeeds = {
  delhi: { temp: 38, feelsLike: 41, humidity: 35, windSpeed: 12, icon: 'sunny', condition: 'Clear sky' },
  bengaluru: { temp: 28, feelsLike: 30, humidity: 76, windSpeed: 16, icon: 'rain', condition: 'Light rain' },
  pune: { temp: 30, feelsLike: 32, humidity: 62, windSpeed: 14, icon: 'cloudy', condition: 'Cloudy' },
  chennai: { temp: 33, feelsLike: 38, humidity: 78, windSpeed: 20, icon: 'partly-cloudy', condition: 'Humid clouds' },
}

const buildDemoWeather = (city = 'Mumbai') => {
  const key = String(city || 'Mumbai').trim().toLowerCase()
  const seed = citySeeds[key] || {}
  return {
    ...DEMO_WEATHER,
    location: {
    ...
    current: {
      ...DEMO_WEATHER.current,
      ...seed,
    },
    hourly: DEMO_WEATHER.hourly.map((hour, index) => ({
      ...hour,
      temp: Math.max(18, Math.round((seed.temp || DEMO_WEATHER.current.temp) - index * 0.5)),
      icon: index > 3 ? 'cloudy' : seed.icon || hour.icon,
    })),
  }
}

module.exports = { DEMO_WEATHER, buildDemoWeather }
