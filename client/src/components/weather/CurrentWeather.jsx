...
const CurrentWeather = ({ weather }) => {
  return (
    <section className="weather-card glass-panel min-h-[320px] p-4 sm:p-5">
      <div className="flex h-full min-w-0 flex-col justify-between gap-6">
        <div className="flex items-start justify-between gap-3 sm:gap-4">
          <div className="min-w-0">
            ...
            <h1 className="break-words text-4xl font-black text-white sm:text-5xl">{weather.location.city}</h1>
            <p className="mt-2 text-sm text-white/72">
              {formatLocalDate(weather.location.localTime)} | {formatShortTime(weather.location.localTime)}
            </p>
          </div>
          <span className="flex-none rounded-lg bg-emerald-300/18 px-3 py-2 text-xs font-semibold text-emerald-100 sm:text-sm">
        ...
        <div className="flex flex-wrap items-center justify-between gap-5">
          <div className="min-w-0">
            <div className="text-6xl font-black leading-none text-white sm:text-7xl lg:text-8xl">
          ...
          <WeatherIcon icon={weather.current.icon} className="weather-icon h-24 w-24 text-amber-200 sm:h-32 sm:w-32 lg:h-36 lg:w-36" aria-hidden="true" />
        </div>
